...
function ic_process_current_products(
	...
	if ( empty( $_GET['s'] ) && empty( $args['al_product-cat'] ) && ( empty( $args ['tax_query'] ) || ( ! empty( $args ['tax_query'] ) && count( $args ['tax_query'] ) === 1 && ! empty( $args ['tax_query'][0]['taxonomy'] ) && $args ['tax_query'][0]['taxonomy'] === 'language' ) ) && empty( $args ['meta_query'] ) && ! is_ic_only_main_cats() && ! apply_filters( 'ic_force_query_count_calculation', false ) ) {
		return 'all';
	}
...
function ic_wp_query( $args, $cached = false ) {
	...
	$current_query = new WP_Query( apply_filters( 'ic_wp_query_args', $final_args ) );
